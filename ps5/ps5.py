...
def generate_models(x, y, degs):
    ...
    for deg in degs:
        coefficients.append(pylab.polyfit(x, y, deg))
...
def r_squared(y, estimated):
    ...
    """
    mean_measured = pylab.mean(y)

    numerator = pylab.sum((y-estimated)**2)
    denom = pylab.sum((y-mean_measured)**2)
...
def moving_average(y, window_length):
    ...
    for i in range(len(y)):
        if i < window_length - 1:
            averages.append(sum(y[0:i+1])/(i+1))
        else:
            averages.append(sum(y[i+1-window_length: i+1])/window_length)
...
def calc_average_temp_of_year(data_years):
    avg = []
...
if __name__ == '__main__':
    ...
    nyc_at_jan10 = []
    years = []
    years.extend(TRAINING_INTERVAL)

    for year in TRAINING_INTERVAL:
        nyc_at_jan10.append(climate.get_daily_temp(
            "NEW YORK", 1, 10, year))
        nyc_yearly.append(climate.get_yearly_temp("NEW YORK", year))

    average_year = calc_average_temp_of_year(nyc_yearly)
