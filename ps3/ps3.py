...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        for wi in range(width):
            ...
            for i in range(height):
                self.tiles[wi].append(dirt_amount)
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        """
        width = math.floor(pos.get_x())
        height = math.floor(pos.get_y())

        dirty_at_pos = self.tiles[width][height]

        if dirty_at_pos - capacity <= 0:
            self.tiles[width][height] = 0
        else:
            self.tiles[width][height] = dirty_at_pos - capacity
    ...
    def get_num_cleaned_tiles(self):
        ...
        for tile in self.tiles:
            for ni in range(len(tile)):
                if self.is_tile_cleaned(m, ni):
                    cleaned_tiles += 1
            m += 1
    ...
    def is_position_in_room(self, pos):
        ...
        if pos.get_x() < 0 or pos.get_y() < 0:
            return False
        return pos.get_x() < self.width and pos.get_y() < self.height
    ...
    def get_coverage(self):
        ...
        return self.get_num_cleaned_tiles()/self.get_num_tiles()
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        """
        self.pos = Position(random.uniform(0, room.width),
                            random.uniform(0, room.height))

        self.tile = (self.pos.get_x(), self.pos.get_y())
        self.speed = speed
    ...
    def is_hit(self):
        if self.room.is_position_valid(self.pos.get_new_position(self.direction, self.speed)):
            return False
        else:
            return True
...
class EmptyRoom(RectangularRoom):
    ...
    def get_random_position(self):
        ...
        """
        pos = Position(random.uniform(0, self.width),
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_valid(self, pos):
        ...
        if pos.get_x() < 0 or pos.get_y() < 0:
            return False
        elif self.is_position_furnished(pos):
            return False
        return pos.get_x() < self.width and pos.get_y() < self.height
    ...
    def get_random_position(self):
        ...
        for furniture in self.furniture_tiles:
            furniture_width.append(furniture[0])
            furniture_heights.append(furniture[1])

        height = random.choice(
            [i for i in range(0, self.height) if i not in furniture_heights])
        width = random.choice(
            [i for i in range(0, self.width) if i not in furniture_width])
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        current_pos = self.pos.get_new_position(self.direction, self.speed)

        if self.room.is_position_valid(current_pos):
...
class FaultyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        if self.gets_faulty():
            ...
            if not self.is_hit():
                self.pos = self.pos.get_new_position(
...
print('avg time steps: ' + str(run_simulation(1,
      1.0, 1, 5, 5, 3, 1.0, 50, StandardRobot)))
print('avg time steps: ' + str(run_simulation(1,
...
#
# 1How does the performance of the two robot types compare when cleaning 80%
#       of a 20x20 room?
# ANSWER: the Faulty Robot is slight less eficcient in this case
#
